...
class SmartConversationExtractor:
    ...
    def _is_english_segment(self, text):
        """세그먼트가 주로 영어인지 판단"""
        text = text.strip()
        if not text:
            return False
    ...
    def find_anchor_and_extract_smart(self, audio_path,
        ...
        # 3. Whisper 전사로 영어 대화 종료 시점 찾기
        print("📝 전사 분석을 통한 영어 대화 구간 탐지 중...")
        
        english_end_time = None
        korean_consecutive = 0  # 연속된 한국어 세그먼트 수
        korean_threshold = 2    # 연속 2개 이상 한국어면 종료
        ...
        for segment in segments:
            ...
            if seg_start >= anchor_end_time:
                is_english = self._is_english_segment(text)
                
                if is_english:
                    # 영어 세그먼트: 계속 포함
                    english_end_time = seg_end
                    korean_consecutive = 0
                    print(f"  ✅ 영어: {seg_start:.1f}초 - {text[:50]}")
                else:
                    # 한국어 세그먼트
                    korean_consecutive += 1
                    print(f"  🇰🇷 한국어: {seg_start:.1f}초 - {text[:50]}")
                    
                    if korean_consecutive >= korean_threshold:
                        print(f"\n  ⏹️  연속 {korean_consecutive}개 한국어 세그먼트 감지, 영어 대화 종료")
                        break
        
        if english_end_time is None or english_end_time <= anchor_end_time:
            print("⚠️  영어 대화 구간을 찾지 못했습니다. 고정 시간 추출합니다.")
            return self._extract_fixed(audio_path, anchor_end_time, base_name, result)
        
        # 4. inaSpeechSegmenter로 음악 시작점 찾기 (선택사항)
        extract_start = anchor_end_time
        
        if HAS_INA and self.segmenter is not None:
            print("\n🎼 음악 및 음성 세그먼트 분석으로 시작점 미세 조정 중...")
            ina_segments = self.segmenter(audio_path)
            
            # 앵커 이후 첫 음악/음성 찾기
            for label, start, end in ina_segments:
                if start >= anchor_end_time and label in ['music', 'male', 'female']:
                    extract_start = start
                    print(f"  🎵 음악/대화 시작점: {extract_start:.2f}초")
                    break
