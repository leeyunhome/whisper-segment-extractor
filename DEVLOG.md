...
- 2026-01-05: Phase 7 추출 로직 버그 수정 (시작점/종료점 개선)

---

### Phase 7: 추출 로직 디버깅 및 개선 (Bug Fix)
**문제점**:
- 빠른 영어 대화가 "한국어 선생님 설명"으로 오인되어 조기 종료됨 (세그먼트 간격이 짧음)
- "음악 후 시작" 요청이 있었으나 음악 시작점부터 추출됨

**해결**:
1. **시작점 로직 변경**:
   - `music` 세그먼트가 끝나고 다음 세그먼트가 시작되는 시점을 시작점으로 설정 (User request: "음악 후 시작")
   
2. **종료점 감지 로직 강화**:
   - 기존: 세그먼트 시작점 간격(Start-to-Start)만 확인 < 5s
   - 변경: **종료점-시작점 간격(End-to-Start)** 확인 (실제 묵음 gap) < 3s ~ 5s
   - **한국어 문자 포함 여부 확인**: `seg['text']`에 한글이 포함된 경우에만 설명 구간으로 판단

**결과**:
- 20260102 파일 기준, 영어 대화가 촘촘하게 이어져도 종료되지 않고, 실제 한국어 설명이 나올 때 정확히 종료됨.
